...
const uint8_T AutopilotLaws_IN_NO_ACTIVE_CHILD_e = 0U;
...
void AutopilotLawsModelClass::AutopilotLaws_Chart_f_Init(rtDW_Chart_AutopilotLaws_k_T *localDW)
{
  localDW->is_active_c1_AutopilotLaws = 0U;
  localDW->is_c1_AutopilotLaws = AutopilotLaws_IN_NO_ACTIVE_CHILD_e;
}

void AutopilotLawsModelClass::AutopilotLaws_Chart_j(real_T rtu_right, real_T rtu_left, boolean_T rtu_use_short_path,
  real_T *rty_out, rtDW_Chart_AutopilotLaws_k_T *localDW)
{
  real_T tmp;
  real_T tmp_0;
  boolean_T tmp_1;
     ...
     case AutopilotLaws_IN_any_o:
      ...
      if (tmp_1 && (tmp < tmp_0) && (tmp >= 10.0) && (tmp <= 20.0)) {
        ...
        *rty_out = rtu_right;
        ...
        localDW->is_c1_AutopilotLaws = AutopilotLaws_IN_left_l;
        *rty_out = rtu_left;
      } else if (tmp_0 < tmp) {
        *rty_out = rtu_left;
      } else {
...
void AutopilotLawsModelClass::step()
{
  ...
  real_T rtb_Sum_bs_tmp;
  rtb_Compare_ef = ((AutopilotLaws_U.in.input.enabled_AP1 != 0.0) || (AutopilotLaws_U.in.input.enabled_AP2 != 0.0));
  ...
  rtb_Mod1_h = 0.017453292519943295 * rtb_GainTheta1;
  rtb_out_l = std::tan(rtb_Saturation1);
  rtb_Saturation = std::sin(rtb_Mod1_h);
  rtb_Mod1_h = std::cos(rtb_Mod1_h);
  tmp[0] = 1.0;
  tmp[3] = rtb_Saturation * rtb_out_l;
  tmp[6] = rtb_Mod1_h * rtb_out_l;
  tmp[1] = 0.0;
  tmp[4] = rtb_Mod1_h;
  ...
  rtb_out_l = AutopilotLaws_P.Gain_Gain_m * AutopilotLaws_U.in.data.r_rad_s;
  for (rtb_on_ground = 0; rtb_on_ground < 3; rtb_on_ground++) {
    result[rtb_on_ground] = tmp[rtb_on_ground + 6] * rtb_out_l + (tmp[rtb_on_ground + 3] * rtb_Saturation1 +
  ...
  AutopilotLaws_Chart(rtb_Saturation1, AutopilotLaws_P.Gain_Gain_h * rt_modd(AutopilotLaws_P.Constant3_Value_g -
    rtb_Saturation1, AutopilotLaws_P.Constant3_Value_g), AutopilotLaws_P.Constant2_Value, &rtb_out_l,
                      &AutopilotLaws_DWork.sf_Chart_jd);
  if (AutopilotLaws_U.in.data.H_radio_ft <= AutopilotLaws_P.CompareToConstant_const) {
    rtb_Saturation1 = AutopilotLaws_P.Gain_Gain_a * rtb_out_l;
  } else {
    rtb_Saturation1 = AutopilotLaws_P.Constant1_Value;
  }

  rtb_out_l = AutopilotLaws_U.in.time.dt * AutopilotLaws_P.LagFilter1_C1;
  rtb_Minup = rtb_out_l + AutopilotLaws_P.Constant_Value_p;
  AutopilotLaws_DWork.Delay1_DSTATE = 1.0 / rtb_Minup * (AutopilotLaws_P.Constant_Value_p - rtb_out_l) *
    AutopilotLaws_DWork.Delay1_DSTATE + (rtb_Saturation1 + AutopilotLaws_DWork.Delay_DSTATE) * (rtb_out_l / rtb_Minup);
  ...
  rtb_out_l = std::sin(AutopilotLaws_P.Gain1_Gain_nb * AutopilotLaws_U.in.data.nav_loc_error_deg) * rtb_Mod2 *
    AutopilotLaws_P.Gain2_Gain_g;
  if (rtb_out_l > AutopilotLaws_P.Saturation1_UpperSat_g) {
    rtb_out_l = AutopilotLaws_P.Saturation1_UpperSat_g;
  } else {
    if (rtb_out_l < AutopilotLaws_P.Saturation1_LowerSat_k) {
      rtb_out_l = AutopilotLaws_P.Saturation1_LowerSat_k;
    }
  }
  ...
  AutopilotLaws_DWork.Delay_DSTATE_h += AutopilotLaws_P.Gain6_Gain * rtb_out_l *
  ...
  rtb_Mod1_h = AutopilotLaws_U.in.data.nav_loc_error_deg + AutopilotLaws_U.in.data.nav_loc_deg;
  rtb_Mod1_fo = rt_modd((AutopilotLaws_U.in.data.Psi_magnetic_deg - (rt_modd(rt_modd(rtb_Mod1_h,
  ...
  rtb_Mod2 = rt_modd(AutopilotLaws_P.Constant3_Value_b - rtb_Mod1_fo, AutopilotLaws_P.Constant3_Value_b);
  if (rtb_Mod1_fo < rtb_Mod2) {
    rtb_Mod1_fo *= AutopilotLaws_P.Gain1_Gain_l;
  } else {
    rtb_Mod1_fo = AutopilotLaws_P.Gain_Gain_j * rtb_Mod2;
  }

  rtb_out_l = rt_modd((rt_modd(rt_modd(((rtb_out_l * look1_binlxpw(AutopilotLaws_U.in.data.V_gnd_kn,
    AutopilotLaws_P.ScheduledGain_BreakpointsForDimension1, AutopilotLaws_P.ScheduledGain_Table, 2U) +
    AutopilotLaws_DWork.Delay_DSTATE_h) + AutopilotLaws_P.Gain1_Gain_f * rtb_Mod1_fo) +
    AutopilotLaws_U.in.data.Psi_magnetic_deg, AutopilotLaws_P.Constant3_Value_ds) + AutopilotLaws_P.Constant3_Value_ds,
    AutopilotLaws_P.Constant3_Value_ds) - (AutopilotLaws_U.in.data.Psi_magnetic_deg + AutopilotLaws_P.Constant3_Value_i))
                      + AutopilotLaws_P.Constant3_Value_i, AutopilotLaws_P.Constant3_Value_i);
  AutopilotLaws_Chart(rtb_out_l, AutopilotLaws_P.Gain_Gain_i * rt_modd(AutopilotLaws_P.Constant3_Value_i - rtb_out_l,
    AutopilotLaws_P.Constant3_Value_i), AutopilotLaws_P.Constant1_Value_e, &rtb_Mod1_fo, &AutopilotLaws_DWork.sf_Chart_n);
  switch (static_cast<int32_T>(rtb_ManualSwitch)) {
   case 0:
    rtb_out_l = AutopilotLaws_P.beta_Value_ed;
    break;

   case 1:
    rtb_out_l = AutopilotLaws_P.beta_Value_e;
    break;

   case 2:
    rtb_out_l = AutopilotLaws_P.beta_Value_b;
    break;

   case 3:
    rtb_out_l = AutopilotLaws_P.beta_Value_m;
    break;

   case 4:
    rtb_out_l = AutopilotLaws_P.beta_Value;
    break;

   case 5:
    rtb_out_l = AutopilotLaws_DWork.Delay1_DSTATE;
    break;

   default:
    rtb_out_l = (AutopilotLaws_P.Gain5_Gain * rtb_Mod1_fo + AutopilotLaws_P.Gain_Gain_b * result[2]) + rtb_Saturation;
    break;
  }
  ...
  rtb_Mod1_fo = rt_modd((AutopilotLaws_U.in.input.Psi_c_deg - (AutopilotLaws_U.in.data.Psi_magnetic_deg +
  ...
  if (rtb_Divide_oz > 100.0) {
    ...
    if (rtb_Divide_oz < 0.0) {
      rtb_Mod2 = std::ceil(rtb_Divide_oz);
    } else {
      rtb_Mod2 = std::floor(rtb_Divide_oz);
    }
  ...
  AutopilotLaws_Chart_j(rtb_Mod1_fo, AutopilotLaws_P.Gain_Gain_o * rt_modd(AutopilotLaws_P.Constant3_Value_a -
    rtb_Mod1_fo, AutopilotLaws_P.Constant3_Value_a), rtb_Compare_c != rtb_Compare_k0, &rtb_Divide_oz,
                        &AutopilotLaws_DWork.sf_Chart_j);
  rtb_Mod1_fo = rt_modd((AutopilotLaws_U.in.input.Psi_c_deg - (AutopilotLaws_U.in.data.Psi_magnetic_track_deg +
  ...
  if (rtb_Divide_epe > 100.0) {
    ...
    if (rtb_Divide_epe < 0.0) {
      rtb_Mod2 = std::ceil(rtb_Divide_epe);
    } else {
      rtb_Mod2 = std::floor(rtb_Divide_epe);
    }
  ...
  AutopilotLaws_Chart_j(rtb_Mod1_fo, AutopilotLaws_P.Gain_Gain_ms * rt_modd(AutopilotLaws_P.Constant3_Value_e -
    rtb_Mod1_fo, AutopilotLaws_P.Constant3_Value_e), rtb_Compare_k0 != rtb_Divide_g, &rtb_Divide_epe,
  ...
  rtb_Mod1_fo = rt_modd((rt_modd(rt_modd((AutopilotLaws_P.Gain2_Gain_f * AutopilotLaws_U.in.data.flight_guidance_tae_deg
  ...
  AutopilotLaws_Chart(rtb_Mod1_fo, AutopilotLaws_P.Gain_Gain_l * rt_modd(AutopilotLaws_P.Constant3_Value_h - rtb_Mod1_fo,
    AutopilotLaws_P.Constant3_Value_h), AutopilotLaws_P.Constant_Value_c, &rtb_out_k, &AutopilotLaws_DWork.sf_Chart);
  rtb_Mod1_h = rt_modd((AutopilotLaws_U.in.data.Psi_magnetic_track_deg - (rt_modd(rt_modd(rtb_Mod1_h,
    AutopilotLaws_P.Constant3_Value_el) + AutopilotLaws_P.Constant3_Value_el, AutopilotLaws_P.Constant3_Value_el) +
    AutopilotLaws_P.Constant3_Value_f)) + AutopilotLaws_P.Constant3_Value_f, AutopilotLaws_P.Constant3_Value_f);
  rtb_Mod1_fo = rt_modd(AutopilotLaws_P.Constant3_Value_f - rtb_Mod1_h, AutopilotLaws_P.Constant3_Value_f);
  ...
  rtb_Mod2 = std::sin(AutopilotLaws_P.Gain1_Gain_e * AutopilotLaws_U.in.data.nav_loc_error_deg) * rtb_Mod2 *
  ...
  if (rtb_Mod1_h < rtb_Mod1_fo) {
    rtb_Mod1_h *= AutopilotLaws_P.Gain1_Gain;
  } else {
    rtb_Mod1_h = AutopilotLaws_P.Gain_Gain * rtb_Mod1_fo;
  }

  rtb_Mod1_h = rt_modd((rt_modd(rt_modd((rtb_Mod2 + rtb_Mod1_h) * AutopilotLaws_P.Gain3_Gain +
  ...
  AutopilotLaws_Chart(rtb_Mod1_h, AutopilotLaws_P.Gain_Gain_jv * rt_modd(AutopilotLaws_P.Constant3_Value_c - rtb_Mod1_h,
    AutopilotLaws_P.Constant3_Value_c), AutopilotLaws_P.Constant_Value_e, &rtb_Mod2, &AutopilotLaws_DWork.sf_Chart_e);
  rtb_Mod1_h = AutopilotLaws_P.DiscreteDerivativeVariableTs_Gain * AutopilotLaws_U.in.data.nav_loc_error_deg;
  rtb_Mod1_fo = (rtb_Mod1_h - AutopilotLaws_DWork.Delay_DSTATE_m) / AutopilotLaws_U.in.time.dt *
    AutopilotLaws_P.Gain3_Gain_i + AutopilotLaws_U.in.data.nav_loc_error_deg;
  rtb_Mod1_m = AutopilotLaws_U.in.time.dt * AutopilotLaws_P.LagFilter_C1;
  rtb_Minup = rtb_Mod1_m + AutopilotLaws_P.Constant_Value_p3;
  AutopilotLaws_DWork.Delay1_DSTATE_i = 1.0 / rtb_Minup * (AutopilotLaws_P.Constant_Value_p3 - rtb_Mod1_m) *
    AutopilotLaws_DWork.Delay1_DSTATE_i + (rtb_Mod1_fo + AutopilotLaws_DWork.Delay_DSTATE_c) * (rtb_Mod1_m / rtb_Minup);
  rtb_Mod1_m = rt_modd((rt_modd(rt_modd(AutopilotLaws_DWork.Delay1_DSTATE_i * look1_binlxpw
    ...
    AutopilotLaws_P.Constant3_Value_j) + AutopilotLaws_P.Constant3_Value_j, AutopilotLaws_P.Constant3_Value_j) -
                        (AutopilotLaws_U.in.data.Psi_magnetic_track_deg + AutopilotLaws_P.Constant3_Value_fx)) +
                       AutopilotLaws_P.Constant3_Value_fx, AutopilotLaws_P.Constant3_Value_fx);
  AutopilotLaws_Chart(rtb_Mod1_m, AutopilotLaws_P.Gain_Gain_nr * rt_modd(AutopilotLaws_P.Constant3_Value_fx - rtb_Mod1_m,
    AutopilotLaws_P.Constant3_Value_fx), AutopilotLaws_P.Constant_Value_p1, &rtb_Minup, &AutopilotLaws_DWork.sf_Chart_h);
  switch (static_cast<int32_T>(rtb_ManualSwitch)) {
   case 0:
    rtb_Mod1_m = rtb_GainTheta1;
    break;

   case 1:
    rtb_Mod1_m = rtb_Divide_oz * look1_binlxpw(AutopilotLaws_U.in.data.V_tas_kn,
      AutopilotLaws_P.ScheduledGain_BreakpointsForDimension1_p, AutopilotLaws_P.ScheduledGain_Table_i, 5U);
    break;

   case 2:
    rtb_Mod1_m = rtb_Divide_epe * look1_binlxpw(AutopilotLaws_U.in.data.V_tas_kn,
      AutopilotLaws_P.ScheduledGain_BreakpointsForDimension1_n, AutopilotLaws_P.ScheduledGain_Table_m, 5U);
    break;

   case 3:
    rtb_Mod1_m = rtb_out_k * look1_binlxpw(AutopilotLaws_U.in.data.V_tas_kn,
      AutopilotLaws_P.ScheduledGain_BreakpointsForDimension1_l, AutopilotLaws_P.ScheduledGain_Table_o, 5U);
    break;

   case 4:
    rtb_Mod1_m = rtb_Mod2 * look1_binlxpw(AutopilotLaws_U.in.data.V_tas_kn,
      AutopilotLaws_P.ScheduledGain_BreakpointsForDimension1_ad, AutopilotLaws_P.ScheduledGain_Table_e, 5U);
    break;

   case 5:
    rtb_Mod1_m = rtb_Minup * look1_binlxpw(AutopilotLaws_U.in.data.V_tas_kn,
      AutopilotLaws_P.ScheduledGain_BreakpointsForDimension1_pm, AutopilotLaws_P.ScheduledGain_Table_a, 5U);
    break;

   default:
    rtb_Mod1_m = AutopilotLaws_P.Constant3_Value;
    break;
  }

  if (rtb_Mod1_m > rtb_ROLLLIM1) {
    rtb_Mod1_m = rtb_ROLLLIM1;
  } else {
    rtb_Mod2 = AutopilotLaws_P.Gain1_Gain_lt * rtb_ROLLLIM1;
  ...
  rtb_Mod2 = rtb_Mod1_m - AutopilotLaws_DWork.Delay_DSTATE_hc;
  ...
  rtb_Minup = rtb_Y + AutopilotLaws_P.Constant_Value_j;
  AutopilotLaws_DWork.Delay1_DSTATE_o = 1.0 / rtb_Minup * (AutopilotLaws_P.Constant_Value_j - rtb_Y) *
    AutopilotLaws_DWork.Delay1_DSTATE_o + (AutopilotLaws_DWork.Delay_DSTATE_hc + AutopilotLaws_DWork.Delay_DSTATE_i) *
    (rtb_Y / rtb_Minup);
  rtb_Mod2 = static_cast<real_T>(rtb_BusAssignment_output_ap_on) - AutopilotLaws_DWork.Delay_DSTATE_b;
  ...
  if (AutopilotLaws_DWork.Delay_DSTATE_b > AutopilotLaws_P.Saturation_UpperSat_n) {
    rtb_Sum2_ko = AutopilotLaws_P.Saturation_UpperSat_n;
  } else if (AutopilotLaws_DWork.Delay_DSTATE_b < AutopilotLaws_P.Saturation_LowerSat_c) {
    rtb_Sum2_ko = AutopilotLaws_P.Saturation_LowerSat_c;
  } else {
    rtb_Sum2_ko = AutopilotLaws_DWork.Delay_DSTATE_b;
  }

  rtb_Mod2 = AutopilotLaws_DWork.Delay1_DSTATE_o * rtb_Sum2_ko;
  rtb_Sum2_ko = AutopilotLaws_P.Constant_Value_p0 - rtb_Sum2_ko;
  rtb_Sum2_ko *= rtb_GainTheta1;
  AutopilotLaws_Y.out.output.autopilot.Phi_c_deg = rtb_Mod2 + rtb_Sum2_ko;
  AutopilotLaws_Y.out.output.flight_director.Phi_c_deg = rtb_Mod1_m;
  if (AutopilotLaws_P.Constant1_Value_h > AutopilotLaws_P.Switch_Threshold_l) {
    rtb_Sum2_ko = (AutopilotLaws_U.in.input.V_c_kn - AutopilotLaws_U.in.data.V_ias_kn) * AutopilotLaws_P.Gain1_Gain_b;
    if (rtb_Sum2_ko > AutopilotLaws_P.Saturation1_UpperSat) {
      rtb_Sum2_ko = AutopilotLaws_P.Saturation1_UpperSat;
    } else {
      if (rtb_Sum2_ko < AutopilotLaws_P.Saturation1_LowerSat) {
        rtb_Sum2_ko = AutopilotLaws_P.Saturation1_LowerSat;
      }
    }
  } else {
    rtb_Sum2_ko = AutopilotLaws_P.Constant1_Value_h;
  }
  ...
  rtb_Mod1_m = rtb_Y + AutopilotLaws_P.Constant_Value_a;
  AutopilotLaws_DWork.Delay1_DSTATE_a = 1.0 / rtb_Mod1_m * (AutopilotLaws_P.Constant_Value_a - rtb_Y) *
    AutopilotLaws_DWork.Delay1_DSTATE_a + (rtb_ManualSwitch + AutopilotLaws_DWork.Delay_DSTATE_k) * (rtb_Y / rtb_Mod1_m);
  rtb_Sum2_ko += AutopilotLaws_P.Gain_Gain_f * AutopilotLaws_DWork.Delay1_DSTATE_a;
  if (rtb_Sum2_ko > AutopilotLaws_P.Saturation_UpperSat_nu) {
    rtb_Sum2_ko = AutopilotLaws_P.Saturation_UpperSat_nu;
  } else {
    if (rtb_Sum2_ko < AutopilotLaws_P.Saturation_LowerSat_d) {
      rtb_Sum2_ko = AutopilotLaws_P.Saturation_LowerSat_d;
    }
  }

  rtb_Sum2_ko -= AutopilotLaws_U.in.data.H_dot_ft_min;
  ...
  rtb_Mod2 = AutopilotLaws_P.ftmintoms_Gain * rtb_Sum2_ko / rtb_Mod2;
  if (rtb_Mod2 > 1.0) {
    rtb_Mod2 = 1.0;
  } else {
    if (rtb_Mod2 < -1.0) {
  ...
  rtb_Gain_kt = AutopilotLaws_P.Gain_Gain_op * std::asin(rtb_Mod2);
  rtb_Divide_epe = rtb_GainTheta - std::cos(AutopilotLaws_P.Gain1_Gain_bz * rtb_GainTheta1) *
    AutopilotLaws_U.in.data.alpha_deg;
  rtb_Y = AutopilotLaws_U.in.time.dt * AutopilotLaws_P.LagFilter1_C1_a;
  rtb_Mod1_m = rtb_Y + AutopilotLaws_P.Constant_Value_l;
  AutopilotLaws_DWork.Delay1_DSTATE_iy = 1.0 / rtb_Mod1_m * (AutopilotLaws_P.Constant_Value_l - rtb_Y) *
    AutopilotLaws_DWork.Delay1_DSTATE_iy + (AutopilotLaws_U.in.data.nav_gs_error_deg +
    AutopilotLaws_DWork.Delay_DSTATE_k2) * (rtb_Y / rtb_Mod1_m);
  rtb_ROLLLIM1 = AutopilotLaws_P.DiscreteDerivativeVariableTs_Gain_l * AutopilotLaws_DWork.Delay1_DSTATE_iy;
  rtb_Divide_oz = (rtb_ROLLLIM1 - AutopilotLaws_DWork.Delay_DSTATE_g) / AutopilotLaws_U.in.time.dt *
    AutopilotLaws_P.Gain3_Gain_o + AutopilotLaws_DWork.Delay1_DSTATE_iy;
  rtb_Sum2_ko = AutopilotLaws_U.in.time.dt * AutopilotLaws_P.LagFilter_C1_n;
  rtb_Y = rtb_Sum2_ko + AutopilotLaws_P.Constant_Value_au;
  AutopilotLaws_DWork.Delay1_DSTATE_k = 1.0 / rtb_Y * (AutopilotLaws_P.Constant_Value_au - rtb_Sum2_ko) *
    AutopilotLaws_DWork.Delay1_DSTATE_k + (rtb_Divide_oz + AutopilotLaws_DWork.Delay_DSTATE_l) * (rtb_Sum2_ko / rtb_Y);
  if (AutopilotLaws_U.in.data.H_radio_ft <= AutopilotLaws_P.CompareToConstant_const_n) {
    rtb_Mod2 = (AutopilotLaws_P.Constant3_Value_d - rtb_Divide_epe) * AutopilotLaws_P.Gain2_Gain;
  } else {
    rtb_Mod2 = AutopilotLaws_DWork.Delay1_DSTATE_k * look1_binlxpw(AutopilotLaws_U.in.data.H_radio_ft,
      AutopilotLaws_P.ScheduledGain_BreakpointsForDimension1_h, AutopilotLaws_P.ScheduledGain_Table_ir, 4U);
  }

  AutopilotLaws_Voter1(rtb_Mod2, AutopilotLaws_P.Gain1_Gain_nq * (AutopilotLaws_P.Constant1_Value_n - rtb_Divide_epe),
                       AutopilotLaws_P.Gain_Gain_p * (AutopilotLaws_P.Constant2_Value_n - rtb_Divide_epe), &rtb_Y_j);
  rtb_Sum_bs_tmp = AutopilotLaws_U.in.input.H_c_ft - AutopilotLaws_U.in.data.H_ind_ft;
  rtb_Mod2 = AutopilotLaws_P.kntoms_Gain_e * AutopilotLaws_U.in.data.V_tas_kn;
  if (rtb_Sum_bs_tmp < 0.0) {
    rtb_Divide_epe = -1.0;
  } else if (rtb_Sum_bs_tmp > 0.0) {
    rtb_Divide_epe = 1.0;
  } else {
    rtb_Divide_epe = rtb_Sum_bs_tmp;
  }

  if (rtb_Mod2 > AutopilotLaws_P.Saturation_UpperSat_a) {
    rtb_Mod2 = AutopilotLaws_P.Saturation_UpperSat_a;
  } else {
    if (rtb_Mod2 < AutopilotLaws_P.Saturation_LowerSat_pd) {
      rtb_Mod2 = AutopilotLaws_P.Saturation_LowerSat_pd;
    }
  }

  rtb_Mod2 = ((AutopilotLaws_P.Constant_Value_b * rtb_Divide_epe + rtb_Sum_bs_tmp) * AutopilotLaws_P.Gain_Gain_e -
  ...
  rtb_Gain_dg = AutopilotLaws_P.Gain_Gain_oe * std::asin(rtb_Mod2);
  rtb_Sum2_ko = AutopilotLaws_P.Gain1_Gain_c * AutopilotLaws_U.in.data.alpha_deg;
  rtb_Y = AutopilotLaws_U.in.data.bz_m_s2 * std::sin(rtb_Sum2_ko);
  rtb_Sum2_ko = std::cos(rtb_Sum2_ko);
  rtb_Sum2_ko *= AutopilotLaws_U.in.data.bx_m_s2;
  ...
  rtb_Sum2_d = (rtb_Y + rtb_Sum2_ko) * AutopilotLaws_P.Gain_Gain_hf * AutopilotLaws_P.Gain_Gain_k + rtb_Mod2;
  ...
  if (rtb_Mod2 > AutopilotLaws_P.Saturation_UpperSat_b) {
    ...
    if (rtb_Mod2 < AutopilotLaws_P.Saturation_LowerSat_h1) {
      rtb_Mod2 = AutopilotLaws_P.Saturation_LowerSat_h1;
    }
  }

  rtb_Mod2 = (AutopilotLaws_P.Constant_Value_k - AutopilotLaws_U.in.data.H_dot_ft_min) *
    AutopilotLaws_P.ftmintoms_Gain_n / rtb_Mod2;
  if (rtb_Mod2 > 1.0) {
    rtb_Mod2 = 1.0;
  } else {
    if (rtb_Mod2 < -1.0) {
  ...
  rtb_Gain_ae = AutopilotLaws_P.Gain_Gain_eu * std::asin(rtb_Mod2);
  rtb_Mod2 = AutopilotLaws_P.kntoms_Gain_a * AutopilotLaws_U.in.data.V_tas_kn;
  if (rtb_Mod2 > AutopilotLaws_P.Saturation_UpperSat_jq) {
    rtb_Mod2 = AutopilotLaws_P.Saturation_UpperSat_jq;
  } else {
  ...
  rtb_Gain_mj = AutopilotLaws_P.Gain_Gain_h0 * std::asin(rtb_Mod2);
  rtb_Sum_l1 = AutopilotLaws_U.in.input.FPA_c_deg - (rtb_GainTheta - std::cos(AutopilotLaws_P.Gain1_Gain_g *
  ...
  if (AutopilotLaws_U.in.data.H_radio_ft > AutopilotLaws_P.Saturation_UpperSat_c) {
    rtb_Divide_epe = AutopilotLaws_P.Saturation_UpperSat_c;
  } else if (AutopilotLaws_U.in.data.H_radio_ft < AutopilotLaws_P.Saturation_LowerSat_a) {
    rtb_Divide_epe = AutopilotLaws_P.Saturation_LowerSat_a;
  } else {
    rtb_Divide_epe = AutopilotLaws_U.in.data.H_radio_ft;
  }

  rtb_Sum2_ko = AutopilotLaws_U.in.time.dt * AutopilotLaws_P.LagFilter2_C1;
  rtb_Y = rtb_Sum2_ko + AutopilotLaws_P.Constant_Value_e5;
  AutopilotLaws_DWork.Delay1_DSTATE_p = 1.0 / rtb_Y * (AutopilotLaws_P.Constant_Value_e5 - rtb_Sum2_ko) *
    AutopilotLaws_DWork.Delay1_DSTATE_p + (rtb_Divide_epe + AutopilotLaws_DWork.Delay_DSTATE_f) * (rtb_Sum2_ko / rtb_Y);
  rtb_Mod1_m = AutopilotLaws_P.DiscreteDerivativeVariableTs_Gain_b * AutopilotLaws_DWork.Delay1_DSTATE_p;
  rtb_out_k = (rtb_Mod1_m - AutopilotLaws_DWork.Delay_DSTATE_p) / AutopilotLaws_U.in.time.dt;
  rtb_Sum2_ko = AutopilotLaws_U.in.time.dt * AutopilotLaws_P.LagFilter_C1_p;
  rtb_Y = rtb_Sum2_ko + AutopilotLaws_P.Constant_Value_o;
  AutopilotLaws_DWork.Delay1_DSTATE_d = 1.0 / rtb_Y * (AutopilotLaws_P.Constant_Value_o - rtb_Sum2_ko) *
    AutopilotLaws_DWork.Delay1_DSTATE_d + (rtb_out_k + AutopilotLaws_DWork.Delay_DSTATE_h3) * (rtb_Sum2_ko / rtb_Y);
  ...
  if (rtb_Mod2 > AutopilotLaws_P.Saturation_UpperSat_ms) {
    rtb_Mod2 = AutopilotLaws_P.Saturation_UpperSat_ms;
  } else {
    if (rtb_Mod2 < AutopilotLaws_P.Saturation_LowerSat_dp) {
      rtb_Mod2 = AutopilotLaws_P.Saturation_LowerSat_dp;
    }
  }

  rtb_Mod2 = (rtb_Y - AutopilotLaws_P.Gain_Gain_jh * AutopilotLaws_DWork.Delay1_DSTATE_d) *
  ...
  rtb_Gain_i1 = AutopilotLaws_P.Gain_Gain_bl * std::asin(rtb_Mod2);
  rtb_Sum_j = AutopilotLaws_P.Constant1_Value_dg - rtb_GainTheta;
  rtb_Sum2_ko = AutopilotLaws_P.Gain1_Gain_d * AutopilotLaws_U.in.data.alpha_deg;
  rtb_Y = AutopilotLaws_U.in.data.bz_m_s2 * std::sin(rtb_Sum2_ko);
  rtb_Sum2_ko = std::cos(rtb_Sum2_ko);
  rtb_Sum2_ko *= AutopilotLaws_U.in.data.bx_m_s2;
  ...
  rtb_Sum2_j = (rtb_Y + rtb_Sum2_ko) * AutopilotLaws_P.Gain_Gain_kj * AutopilotLaws_P.Gain_Gain_ml + rtb_Mod2;
  ...
  rtb_Gain_cb = AutopilotLaws_P.Gain_Gain_cm * std::asin(rtb_Mod2);
  AutopilotLaws_Voter1(rtb_Sum_j, AutopilotLaws_P.Gain_Gain_b2 * rtb_Sum2_j, AutopilotLaws_P.VS_Gain_c * rtb_Gain_cb,
                       &rtb_Sum2_ko);
  switch (static_cast<int32_T>(rtb_Minup)) {
   case 0:
    rtb_Sum2_ko = rtb_GainTheta;
    break;

   case 1:
    rtb_Sum2_ko = AutopilotLaws_P.VS_Gain * rtb_Gain_kt;
    break;

   case 2:
    rtb_Sum2_ko = AutopilotLaws_P.VS_Gain_a * rtb_Gain_dg;
    break;

   case 3:
    if (rtb_Sum_bs_tmp > AutopilotLaws_P.Switch_Threshold_n) {
      rtb_Mod2 = AutopilotLaws_P.Gain_Gain_kg * rtb_Sum2_d;
      rtb_Sum2_ko = AutopilotLaws_P.VS_Gain_j * rtb_Gain_ae;
      if (rtb_Mod2 > rtb_Sum2_ko) {
        rtb_Sum2_ko = rtb_Mod2;
      }
    } else {
      rtb_Mod2 = AutopilotLaws_P.Gain_Gain_kg * rtb_Sum2_d;
      rtb_Sum2_ko = AutopilotLaws_P.VS_Gain_j * rtb_Gain_ae;
   ...
   case 4:
    rtb_Sum2_ko = AutopilotLaws_P.VS_Gain_h * rtb_Gain_mj;
    break;

   case 5:
    rtb_Sum2_ko = AutopilotLaws_P.Gain_Gain_c * rtb_Sum_l1;
    break;
   ...
   case 7:
    if (rtb_on_ground > AutopilotLaws_P.Switch_Threshold) {
      rtb_Sum2_ko = rtb_Gain4;
    } else {
      rtb_Sum2_ko = AutopilotLaws_P.VS_Gain_e * rtb_Gain_i1;
    }
    break;
  }

  rtb_Sum2_ko += rtb_GainTheta;
  if (rtb_Sum2_ko > AutopilotLaws_P.Constant1_Value_i) {
    rtb_Sum2_ko = AutopilotLaws_P.Constant1_Value_i;
  } else {
    rtb_Mod2 = AutopilotLaws_P.Gain1_Gain_m * AutopilotLaws_P.Constant1_Value_i;
    if (rtb_Sum2_ko < rtb_Mod2) {
      rtb_Sum2_ko = rtb_Mod2;
    }
  }
  ...
  rtb_Sum2_ko -= AutopilotLaws_DWork.Delay_DSTATE_h2;
  rtb_Y = AutopilotLaws_P.Constant2_Value_h1 * AutopilotLaws_U.in.time.dt;
  if (rtb_Sum2_ko >= rtb_Y) {
    rtb_Sum2_ko = rtb_Y;
  }
  ...
  rtb_Sum2_ko = AutopilotLaws_U.in.time.dt * AutopilotLaws_P.LagFilter_C1_i;
  rtb_Y = rtb_Sum2_ko + AutopilotLaws_P.Constant_Value_lo;
  AutopilotLaws_DWork.Delay1_DSTATE_px = 1.0 / rtb_Y * (AutopilotLaws_P.Constant_Value_lo - rtb_Sum2_ko) *
    AutopilotLaws_DWork.Delay1_DSTATE_px + (AutopilotLaws_DWork.Delay_DSTATE_h2 + AutopilotLaws_DWork.Delay_DSTATE_o) *
    (rtb_Sum2_ko / rtb_Y);
  rtb_Mod2 = static_cast<real_T>(rtb_BusAssignment_output_ap_on) - AutopilotLaws_DWork.Delay_DSTATE_ch;
  ...
  AutopilotLaws_DWork.Delay_DSTATE_ch += rtb_Mod2;
  if (AutopilotLaws_DWork.Delay_DSTATE_ch > AutopilotLaws_P.Saturation_UpperSat_g) {
    rtb_Sum2_ko = AutopilotLaws_P.Saturation_UpperSat_g;
  } else if (AutopilotLaws_DWork.Delay_DSTATE_ch < AutopilotLaws_P.Saturation_LowerSat_df) {
    rtb_Sum2_ko = AutopilotLaws_P.Saturation_LowerSat_df;
  } else {
    rtb_Sum2_ko = AutopilotLaws_DWork.Delay_DSTATE_ch;
  }

  rtb_Mod2 = AutopilotLaws_DWork.Delay1_DSTATE_px * rtb_Sum2_ko;
  rtb_Sum2_ko = AutopilotLaws_P.Constant_Value_iv - rtb_Sum2_ko;
  rtb_Sum2_ko *= rtb_GainTheta;
  AutopilotLaws_Y.out.output.autopilot.Theta_c_deg = rtb_Mod2 + rtb_Sum2_ko;
  AutopilotLaws_Voter1(rtb_Sum_j, rtb_Sum2_j, rtb_Gain_cb, &rtb_Y);
  switch (static_cast<int32_T>(rtb_Minup)) {
   case 0:
    rtb_Y = rtb_GainTheta;
    break;
   ...
   case 3:
    if (rtb_Sum_bs_tmp > AutopilotLaws_P.Switch_Threshold_k) {
      if (rtb_Sum2_d > rtb_Gain_ae) {
        rtb_Y = rtb_Sum2_d;
      } else {
        rtb_Y = rtb_Gain_ae;
      }
    } else if (rtb_Sum2_d < rtb_Gain_ae) {
      rtb_Y = rtb_Sum2_d;
    } else {
      rtb_Y = rtb_Gain_ae;
    }
  ...
  AutopilotLaws_Y.out.output.ap_on = rtb_Compare_ef;
  AutopilotLaws_Y.out.output.flight_director.Beta_c_deg = rtb_out_l;
  AutopilotLaws_Y.out.output.autopilot.Beta_c_deg = rtb_out_l;
  ...
  AutopilotLaws_DWork.Delay_DSTATE_m = rtb_Mod1_h;
  AutopilotLaws_DWork.Delay_DSTATE_c = rtb_Mod1_fo;
  AutopilotLaws_DWork.icLoad = 0U;
  AutopilotLaws_DWork.Delay_DSTATE_i = AutopilotLaws_DWork.Delay_DSTATE_hc;
  AutopilotLaws_DWork.Delay_DSTATE_k = rtb_ManualSwitch;
  AutopilotLaws_DWork.Delay_DSTATE_k2 = AutopilotLaws_U.in.data.nav_gs_error_deg;
  AutopilotLaws_DWork.Delay_DSTATE_g = rtb_ROLLLIM1;
  AutopilotLaws_DWork.Delay_DSTATE_l = rtb_Divide_oz;
  AutopilotLaws_DWork.Delay_DSTATE_f = rtb_Divide_epe;
  AutopilotLaws_DWork.Delay_DSTATE_p = rtb_Mod1_m;
  AutopilotLaws_DWork.Delay_DSTATE_h3 = rtb_out_k;
  AutopilotLaws_DWork.icLoad_f = 0U;
  AutopilotLaws_DWork.Delay_DSTATE_o = AutopilotLaws_DWork.Delay_DSTATE_h2;
}
...
void AutopilotLawsModelClass::initialize()
{
...

  {
    ...
    AutopilotLaws_DWork.Delay_DSTATE_c = AutopilotLaws_P.Delay_InitialCondition_e;
    AutopilotLaws_DWork.Delay1_DSTATE_i = AutopilotLaws_P.Delay1_InitialCondition_b;
    AutopilotLaws_DWork.icLoad = 1U;
    AutopilotLaws_DWork.Delay_DSTATE_i = AutopilotLaws_P.Delay_InitialCondition_p;
    AutopilotLaws_DWork.Delay1_DSTATE_o = AutopilotLaws_P.Delay1_InitialCondition_o;
    AutopilotLaws_DWork.Delay_DSTATE_b = AutopilotLaws_P.RateLimiterVariableTs_InitialCondition;
    AutopilotLaws_DWork.Delay_DSTATE_k = AutopilotLaws_P.Delay_InitialCondition_g;
    AutopilotLaws_DWork.Delay1_DSTATE_a = AutopilotLaws_P.Delay1_InitialCondition_e;
    AutopilotLaws_DWork.Delay_DSTATE_k2 = AutopilotLaws_P.Delay_InitialCondition_m;
    AutopilotLaws_DWork.Delay1_DSTATE_iy = AutopilotLaws_P.Delay1_InitialCondition_k;
    AutopilotLaws_DWork.Delay_DSTATE_g = AutopilotLaws_P.DiscreteDerivativeVariableTs_InitialCondition_h;
    AutopilotLaws_DWork.Delay_DSTATE_l = AutopilotLaws_P.Delay_InitialCondition_j;
    AutopilotLaws_DWork.Delay1_DSTATE_k = AutopilotLaws_P.Delay1_InitialCondition_i;
    AutopilotLaws_DWork.Delay_DSTATE_f = AutopilotLaws_P.Delay_InitialCondition_i;
    AutopilotLaws_DWork.Delay1_DSTATE_p = AutopilotLaws_P.Delay1_InitialCondition_c;
    AutopilotLaws_DWork.Delay_DSTATE_p = AutopilotLaws_P.DiscreteDerivativeVariableTs_InitialCondition_a;
    AutopilotLaws_DWork.Delay_DSTATE_h3 = AutopilotLaws_P.Delay_InitialCondition_o;
    AutopilotLaws_DWork.Delay1_DSTATE_d = AutopilotLaws_P.Delay1_InitialCondition_f;
    AutopilotLaws_DWork.icLoad_f = 1U;
    AutopilotLaws_DWork.Delay_DSTATE_o = AutopilotLaws_P.Delay_InitialCondition_gl;
    AutopilotLaws_DWork.Delay1_DSTATE_px = AutopilotLaws_P.Delay1_InitialCondition_a;
    AutopilotLaws_DWork.Delay_DSTATE_ch = AutopilotLaws_P.RateLimiterVariableTs_InitialCondition_p;
    ...
    AutopilotLaws_Chart_Init(&AutopilotLaws_DWork.sf_Chart_jd);
    AutopilotLaws_Chart_Init(&AutopilotLaws_DWork.sf_Chart_n);
    AutopilotLaws_Chart_f_Init(&AutopilotLaws_DWork.sf_Chart_j);
    AutopilotLaws_Chart_f_Init(&AutopilotLaws_DWork.sf_Chart_c);
